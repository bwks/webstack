...
pub enum DatabaseError {
    /// `RocksDB` could not be opened at the configured directory.
    ...
    /// The configured namespace and database could not be selected.
    #[error("cannot select database {namespace}/{database}")]
    Select {
        namespace: String,
        database: String,
        #[source]
    ...
    #[error("invalid migration path {}; expected a regular NNNN_lowercase_name.surql file", .0.display())]
